...
const registerTutor = async (req, res) => {
  try {
    ...
    if (tutor && tutor.is_verified) {
      return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Tutor already exists" });
    }
    if (tutor && !tutor.is_verified) {
    ...
    res.status(STATUS_CODES.OK).json({ message: "OTP sent to your email" });
  } catch (error) {
...
const loginTutor = async (req, res) => {
  try {
    ...
    res.cookie('tutorRefreshToken', refreshToken, getCookieOptions());
    res.status(STATUS_CODES.OK).json({
...
const resendTutorOtp = async (req, res) => {
  try {
    ...
    if (tutor.is_verified) {
      return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Tutor is already verified" });
    }
    await sendOtpToEmail(email, 'tutor');
    res.status(STATUS_CODES.OK).json({ message: "A new OTP has been sent to your email." });
  } catch (error) {
