...
export const formatPrice = (price, offerPercentage = 0) => {
  ...
  if (priceData.hasOffer) {
    return `${CURRENCY.SYMBOL}${priceData.discountedPrice.toFixed(0)} (${priceData.offerPercentage}% OFF from ${CURRENCY.SYMBOL}${priceData.originalPrice.toFixed(0)})`;
  }
  return `${CURRENCY.SYMBOL}${priceData.originalPrice.toFixed(0)}`;
};
